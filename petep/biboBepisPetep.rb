...
def translateWord(str, fustr, ind)
  ...
  if str == "rick"
    ...
    if numb == 3 || (str=='mateo' && [1,2,3,4].include?(numb) )
    ...
    elsif numb == 5 || (str=='mateo' && [5,6].include?(numb) )
      tempret = "beter"
      if joest == 5
        tempret += " boestar"
      end
...
def main()
  ...
  if other == "none"
    ind = 0
    stri = ""
    File.foreach(ARGV[0]) {|x| stri += x.to_s }
    comment = false
    stri.downcase.split(" ").each do |str|
      dontadd = false
      if str == "/*"
        comment = true
        dontadd = true
      ...
      if comment == false && dontadd == false
        full += translateWord(str, stri, ind)
        full += " "
      elsif comment == true && dontadd == false
        full += str
        full += " "
      end
      ind+=1
